clear ; close all; clc;%清理工作区的参量，关闭已画图形，清空命令行已输入的指令
parameters;  %调用名为parameters的m文件
peak=100;  %峰值功率
FWHM=2; %半极大全宽度
at_in=sqrt(peak/339)*sech(t/(FWHM*5)); %输入脉冲
aw = ifft(at_in);   %频域
for i=1:1:Cycles    %循环次数
%%%%%%%%%%%% fiber 1 %%%%%%%%%%%%     
...
%%%%%%%%%%%% gain fiber %%%%%%%%%%%%
   ...
   aw = awz(end,1:N);
   if i==Cycles
      evaw =[evaw;awz];     % 此处是分号，矩阵行的追加，每一行表示一个状态
      evz =[evz;z2+L1];     % 位置追加
